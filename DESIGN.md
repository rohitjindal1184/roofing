...
- **The hero — the one authored moment:** the homepage hero is the single place motion is deliberately elevated above the ambient scroll-reveal. On paint (not on intersection, since it's already in view): the eyebrow, headline, body copy, buttons and stat row rise and fade in a ~110ms stagger (`data-hero-in`, `hero-rise` keyframe). The hero photo runs a slow 20s Ken Burns zoom (`scale(1)` → `scale(1.07)`, `.hero-media`) and, independently, drifts via scroll-linked parallax through a wrapping layer (`HeroParallax.tsx`) so the photo appears to move slower than the page as it scrolls away. As the visitor scrolls the hero out of view, `HeroScrollDriver.tsx` writes a 0→1 progress value onto `--hero-p`; the text block scales down and fades (`[data-hero-layer="content"]`) and the scrim deepens slightly (`[data-hero-layer="scrim"]`) so the hero settles into the next section instead of cutting off abruptly. Do not add a second Ken Burns/parallax treatment elsewhere — it stays a hero-only signature. Components live in `src/components/hero/`.
- **Reduced motion:** every animation and transition collapses to near-zero duration under `prefers-reduced-motion: reduce` (global rule in `globals.css`); `HeroParallax` and `HeroScrollDriver` also short-circuit their scroll listeners entirely rather than relying on the CSS override.
